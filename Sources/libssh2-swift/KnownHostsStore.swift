...
public enum HostKeyCheckResult {
    case match
    case notFound(Data, Int32)
    case mismatch(Data, Int32)
}

public struct KnownHostsStore {
    ...
    public static func addOrReplace(session: OpaquePointer, host: String, port: Int, keyData: Data, keyType: Int32, replace: Bool) throws {
        guard let knownHosts = libssh2_knownhost_init(session) else {
            throw SSHError.knownHostsInitFailed
        }
        defer { libssh2_knownhost_free(knownHosts) }
